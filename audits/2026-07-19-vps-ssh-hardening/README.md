...
## Levantamento inicial de egress — feito, política ainda NÃO aplicada

Puxado o fio do "e se a VPS comprometida simplesmente conectar pra fora" (ver nota no Achado 2): nenhuma correção de `sshd_config` impede isso, porque o cliente SSH (ou qualquer outro binário) da própria VPS saindo não passa pelo daemon que endurecemos. Pra fechar esse lado seria preciso controle de egress (equivalente ao `net_guard`/opensnitch já em uso no desktop), mas **isso não foi aplicado nesta auditoria** — só o levantamento inicial, porque bloquear saída às cegas numa VPS com serviço de produção rodando (mail, webhooks, gateway WhatsApp) tem risco real de derrubar algo.

Inventário estático (`docker inspect` das variáveis de ambiente de cada container, filtrando por URL/host, sem expor as chaves em si):

- **contabo1** (`loro-app`, o orquestrador multi-modelo): fala com OpenAI (`auth.openai.com` + OAuth Codex), Gemini, MiniMax, DeepSeek (chaves configuradas, host default do SDK — não aparece em env), Telegram Bot API, Resend (e-mail transacional), Cloudflare, Brave Search, Serper, GitHub, YouTube. Domínio próprio: `api.lorox.com.br`.
- **contabo2**: `n8n` (`n8n.frontzin.com.br`, `webhook.frontzin.com.br`), `evolution-api` fala com `graph.facebook.com` (Meta/WhatsApp Business Graph API) e tem integração Typebot. Domínio próprio: `api.frontzin.com.br`.
- **fomentofacil**: stack `editais-*` (Prometheus/Grafana/Alertmanager/Flower+Celery/Postgres) — nenhuma URL externa explícita nos envs, mas roda Celery (`editais-flower` monitora workers) que tipicamente agenda scraping internamente, fora do cron do SO (`crontab -l`/`/etc/cron.d` checados nas três VPS — só jobs padrão de sistema: `atop`, `debsecan`, `e2scrub`, `sysstat`, `certbot`, `staticroute`, nada de crawler nem em `contabo1`/`contabo2`. Confirma que os "crawlers"/jobs mencionados pelo usuário rodam dentro da aplicação — n8n trigger nodes, Celery beat, scheduler próprio do `loro-app` — não no SO. Não investigado a fundo, fica pendente.

**Por que essa lista sozinha não é suficiente pra montar allowlist**: APIs cujo endpoint é fixo no SDK (não configurável por env) simplesmente não aparecem aqui — é o caso de praticamente todo provedor de LLM citado acima. Uma política de egress `deny outgoing + allowlist` desenhada só com essa lista quebraria a maior parte das integrações reais no primeiro uso. Próximo passo necessário: janela de observação de tráfego real (`ss`/`conntrack` periódico) antes de qualquer bloqueio.

**Achado lateral (mesma família do vetor de túnel SSH que motivou a auditoria)**: contabo1 tem um **Cloudflare Quick Tunnel** ativo (`*.trycloudflare.com` + `CLOUDFLARE_BROWSER_TOKEN` nos envs do `loro-app`) — expõe algo local pra internet via conexão de saída disfarçada de HTTPS normal, sem precisar de porta aberta. Não investigado o que está do outro lado nem se é intencional/necessário. Candidato a próxima auditoria, mesma urgência do resto desta pendência de egress.
